...
#     return render(request, 'student/stud_register.html', {'form': form})

# # Student Login View (using Django's built-in authentication views)
...
def change_password(request):
    ...
    emp = models.Admin.objects.get(user=request.user)
    context_dict = {}
    if request.method == 'POST':
...
def home(request):
    ...
    context_dict = {}
    employee = models.Admin.objects.filter(user=request.user).first()
    
    # Get employee info and handle the case where employee is None
    emp_info = context_helper.get_emp_info(employee)
    if emp_info is not None:
        context_dict.update(emp_info)
    else:
        # Handle the case where emp_info is None
        context_dict['error'] = 'Employee information not found'

    print(context_dict)  # Debugging line
...
def logout_view(request):
    ...
    """
    logout(request)
    return HttpResponseRedirect('/login/')  # Redirect to the login URL
...
def bar_chart(request):
    ...
    # Query the company names through the campus_drive relation
    pname = models.Placements.objects.filter(
        soft_delete=False
    ).values_list('campus_drive__company__name', flat=True)

    # Query the package values directly from Placements
    package = models.Placements.objects.filter(
        soft_delete=False
...
def pie_chart(request):
    """
    Analytics for student and company placement data.
    """
    # Retrieve the company names associated with each placement through campus_drive
    pdata = models.Placements.objects.filter(soft_delete=False).select_related('campus_drive').values_list('campus_drive__company__name', flat=True)
    
    # Count the frequency of placements per company
    company_counts = Counter(pdata)
    
    # Prepare the dataset for rendering in the template
    dataset = {
        'labels': list(company_counts.keys()),  # Company names
        'data': list(company_counts.values())   # Number of students placed in each company
    }
    
    context_dict = {
        'ylabel': 'Companies',
        'dataset': dataset  # Pass dataset to the template
    }
